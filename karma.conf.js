...
module.exports = function(config) {
  ...
  if (process.env.SAUCE_USERNAME && process.env.SAUCE_ACCESS_KEY && process.env.TRAVIS_SECURE_ENV_VARS !== "false") {
    useSauce = true;
  }

  if (useSauce && rocess.env.TEST_BROWSER_NAME && process.env.TEST_BROWSER_NAME != 'PhantomJS') {
    ...
    for (var i = 0; i < names.length; i++) {
      for (var j = 0; j < platforms.length; j++) {
        for (var k = 0; k < versions.length; k++) {
          ...
          if (versions[i]) {
            customLauncher[launcher_name].version = versions[k];
          }
        }
      }
    }
